...
## 2026-06-10 - Using inline defs for small methods in Scala 3\n**Learning:** Using `inline def` for small getter/setter and extension methods, such as `Physics.Vector2`'s `x` and `y`, or `CodePointSyntax`'s `getDisplayWidth` with pure expression blocks can significantly improve execution speed inside hot loops. However, they must be formatted as expressions without early returns to compile successfully.\n**Action:** Use `inline def` for simple pure functions inside or called by hot loops (like render loops) to avoid JVM method call overhead, provided they are structured properly to comply with Scala 3's inlining rules.
