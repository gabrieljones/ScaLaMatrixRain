...
## 2026-03-05 - [Optimization Success: Avoid Long Conversion in Hot Loop]
**Learning:** When calculating the random `newCharIndex` during a matrix "glitch", the original code used `(((r >>> 14).toLong * setsLength.toLong) >>> 17).toInt`. Since `r >>> 14` is strictly bounded to 131071 (max 17 bits) and `setsLength` is reasonably small (much less than 16384 for standard Unicode sets), their product will not exceed the 32-bit signed integer limit (`131071 * 16000 ≈ 2.09B < 2.14B`).
**Insight:** By replacing the 64-bit `.toLong` and `.toInt` conversions with pure 32-bit math `(((r >>> 14) * setsLength) >>> 17)`, we remove the CPU overhead associated with widening/narrowing conversions and register shuffling in the highly-executed glitch logic block.
**Action:** Always verify mathematical bounds for integer operations inside hot loops. If an intermediate multiplication fits within 32 bits, avoid converting to 64-bit longs just to feel "safe", as it incurs a slight but measurable performance penalty.
