...
## 2023-11-20 - [Optimization Success: Arrays of Structures to Structure of Arrays]
**Learning:** Flattening drops arrays (`dropsFlattened`) from Array[Int] of size `dropQuantity * 4` where consecutive items were `x`, `y`, `vx`, `vy` into Structure of Arrays `dropsX`, `dropsY`, `dropsVX`, `dropsVY` and iterating through them linearly improves cache hit ratios and sequential striding.
**Insight:** SoA is particularly impactful on modern architectures because it aligns contiguous loops like those of movement calculations (e.g. iterating arrays `dropsX` and `dropsVX` together). By pulling all `X` components and avoiding constant 4-stride loading and reading, it increases spatial locality. In this codebase, it successfully pushed FPS from ~2500-2600 to ~2700-2800.
**Action:** Use SoA over AoS or flat AoS where components are read individually.
