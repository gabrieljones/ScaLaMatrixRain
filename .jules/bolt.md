...
## 2026-03-05 - [Optimization Success: Fast path for loop iteration]
**Learning:** Replacing a nested while loop for matrix iteration (which incremented `rowOffset` by `cols` at each row) with a single while loop up to `rows * cols` simplifies control flow and slightly improves cache locality. Combining this with explicit fast paths for vector velocities (`vX == 1`, `vX == -1`) avoiding modulo yields a ~4% performance improvement in the hot path.
**Action:** When iterating a full grid that is stored sequentially in memory, use a single flat loop with a flat index `idx` from `0` to `width * height` rather than maintaining manual `x`, `y`, and `rowOffset` unless the specific coordinate computations are required in an inner branch.
