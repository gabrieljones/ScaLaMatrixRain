...
class CodePointSyntaxTest {
  ...
  def testGetDisplayWidth(): Unit = {
    ...
    assertEquals(0, 0x0.asSetsOfCodePointsSingle.get(0).getDisplayWidth)
    assertEquals(0, 0x0300.asSetsOfCodePointsSingle.get(0).getDisplayWidth) // Combining Grave Accent (NON_SPACING_MARK)
    assertEquals(0, 0x200B.asSetsOfCodePointsSingle.get(0).getDisplayWidth) // Zero Width Space

    // Wide characters
    assertEquals(2, 0x3041.asSetsOfCodePointsSingle.get(0).getDisplayWidth) // Hiragana Letter A
    assertEquals(2, 0x1F600.asSetsOfCodePointsSingle.get(0).getDisplayWidth) // Grinning Face Emoji
    assertEquals(2, 0xFF01.asSetsOfCodePointsSingle.get(0).getDisplayWidth) // Fullwidth Exclamation Mark

    // Standard width
    assertEquals(1, 'A'.toInt.asSetsOfCodePointsSingle.get(0).getDisplayWidth)
    assertEquals(1, ' '.toInt.asSetsOfCodePointsSingle.get(0).getDisplayWidth)
  }
  ...
  extension (i: Int) {
    def asSetsOfCodePointsSingle: SetsOfCodePoints = Array(i).asSetsOfCodePoints
  }
}
