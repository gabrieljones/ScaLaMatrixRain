...
## 2026-03-05 - [Optimization Success: Manual Inlining of Helper Methods]
**Learning:** In highly critical rendering loops, extracting helper methods like `updateChar` completely and manually inlining their logic directly in the loop (e.g., reusing pre-calculated coordinates and direct array accesses) is significantly faster than using Scala 3's `inline def` when those methods access surrounding loop scope variables.
**Insight:** While `inline def` at the class level cleanly avoids call overhead, bringing `inline def` inside the render loop scope unexpectedly degraded performance (from ~2500 FPS -> ~900 FPS), likely due to closure capture semantics or preventing JIT from optimizing loop invariants effectively. On the other hand, manually inlining the short array access logic gave a reliable ~2-4% speedup (~2580 FPS).
**Action:** For trivial state updates in the hottest loop paths (e.g., modifying pixel buffers), explicitly unroll and manual inline the few lines of code to eliminate method call overhead, avoid closure captures, and reduce redundant index arithmetic.
