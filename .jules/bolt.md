...
## 2026-06-09 - [Optimization Success: 32-bit Lemire Multiplication]
**Learning:** In the render loop, bounds mapping using Lemire's multiplication-shift method was using 64-bit `.toLong` conversions (`(((r >>> 14).toLong * setsLength.toLong) >>> 17).toInt`). Since the extracted bits `(r >>> 14)` are at most 17 bits (`131071`), and `setsLength` is a small integer (usually < 200), their product (`~26,000,000`) easily fits within a signed 32-bit integer (`Integer.MAX_VALUE` is `~2.14 billion`).
**Insight:** Bypassing 64-bit conversion for arithmetic that is guaranteed not to overflow 32-bit bounds eliminates type conversion overhead and reduces ALU latency in extremely hot inner loops.
**Action:** When mapping numbers using shifts and multiplication in tight loops, statically verify the maximum possible bounds of the operands. If their product safely fits within 31 bits, remove the `.toLong` casts to achieve strictly 32-bit math.
