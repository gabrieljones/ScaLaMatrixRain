...
## 2025-04-13 - [Optimization Success: Avoid redundant 64-bit Long casts]
**Learning:** Replaced `(((r >>> 14).toLong * setsLength.toLong) >>> 17).toInt` with `(((r >>> 14) * setsLength) >>> 17)` in the tight render loop.
**Insight:** A 31-bit random integer shifted right by 14 has a maximum value of 131,071. Multiplied by a small `setsLength` (e.g., 50-100), the result easily fits within a 32-bit signed integer (2,147,483,647). Therefore, the `.toLong` conversions and the subsequent `.toInt` conversion are completely unnecessary and incur overhead in the inner loop. Removing these allowed the math to be evaluated entirely as 32-bit integers without overflow.
**Action:** Always check the upper bounds of integers in multiplication chains inside tight loops. If the maximum possible product is less than `Int.MaxValue`, avoid `.toLong` conversions to reduce overhead.
