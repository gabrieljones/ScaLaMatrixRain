...
## 2026-03-05 - [Optimization Failure: Skipping Rendering for Stationary Drops]
**Learning:** In the drop advancement loop of `Main.scala`, I attempted to skip the random character generation (`nextBounded`) and head character redraw (`updateChar`) if a drop hadn't moved (`pXN == pXC && pYN == pYC`). While this provided a significant benchmark FPS boost (e.g., from ~2750 to ~3150 FPS), it was rejected because it fundamentally altered the visual fidelity of the digital rain effect.
**Insight:** Matrix digital rain relies on rapid, continuous flickering of characters, even when a drop's vertical progression is slow. By tying the character refresh strictly to the movement velocity, stationary or slow-moving drops stopped flickering, visibly breaking the intended aesthetic effect. Furthermore, skipping the redraw of the head while the global fade effect is active can cause stationary drops to incorrectly dim out.
**Action:** Do not sacrifice the correct visual rendering or business logic of an application for micro-optimizations. Performance improvements must preserve the *exact* existing functionality unless an architectural change to the visual effect is explicitly requested and approved.

## 2026-03-05 - [Optimization Success: Avoid OutOfBounds Check for Stationary Drops]
**Learning:** In the drop advancement loop of `Main.scala`, evaluating if a drop has actually moved (`pXN != pXC || pYN != pYC`) *before* checking if the drop is out of bounds (`acceleration.outOfBounds`) prevents wasted CPU cycles on stationary drops.
**Insight:** A drop that hasn't moved since the last frame cannot possibly have suddenly gone out of bounds. Skipping the out-of-bounds check (which often involves boundary comparisons) for stationary drops yields a small but measurable performance improvement. This is safe because boundary violations are fundamentally a consequence of movement.
**Action:** Delay bounds checking until you have proven the entity has actually moved.
