  ...
  def runLoop(options: Options, terminal: Terminal, sets: SetsOfCodePoints, colorContext: ColorContext): Unit = {
    ...
    val frameFn: Runnable = () => {
      ...
      while (dI < dropsLength) {
        ...
        // Optimization: Defer random number generation until after drop movement
        // and boundary checks to prevent unnecessary RNG execution for drops that haven't moved.
        if (pXN != pXC || pYN != pYC) {
          ...
          // If drop hasn't moved, keep head alive so it doesn't fade into the background.
          val existingCharIndex = if (charIndexBuffer(pYN * cols + pXN) >= 0) charIndexBuffer(pYN * cols + pXN) else nextBounded(setsLength)
          updateChar(pXN, pYN, existingCharIndex, 0, cols, rows)
        }
        {
