...
object CodePointSyntax {
  ...
  extension (codePoint: CodePoint) {
    inline def getDisplayWidth: Int = {
      // 1. Check for Zero Width characters (like combiners)
      if (codePoint == 0 || Character.getType(codePoint) == Character.NON_SPACING_MARK || Character.getType(codePoint) == Character.ENCLOSING_MARK || (codePoint >= 0x200B && codePoint <= 0x200F)) 0
      // 2. Check for Wide characters (CJK, Full-width, Emojis)
      // Basic CJK ranges and East Asian Full-width forms
      else if ((codePoint >= 0x1100 && codePoint <= 0x115F) || (codePoint >= 0x2329 && codePoint <= 0x232A) || (codePoint >= 0x2E80 && codePoint <= 0x303E) || // CJK Radicals
