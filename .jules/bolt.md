...
## 2026-03-24 - [Optimization Success: Inline Method in Opaque Types & Vector2]
**Learning:** Adding the `inline` keyword to extension methods on `opaque type` implementations (like `CodePointSyntax` methods `getDisplayWidth`, `length`, `get`, `getAsInt`, and `Physics.Vector2` extension methods `x` and `y`) resulted in a massive FPS boost, improving from ~2250 FPS to ~2800 FPS in the unthrottled benchmark (+24%).
**Insight:** While the JVM HotSpot JIT compiler usually inlines small methods at runtime, Scala 3's `inline` keyword for extension methods on opaque types guarantees that the method body is expanded at compile time. This is especially critical in tight loops where the overhead of an extension method call (or the compiler's representation of it before JIT kicks in) can still accumulate, or where JIT fails to optimize due to call depth or perceived complexity.
**Action:** Always consider using `inline` on simple, frequently accessed extension methods (like accessors or basic math) on Scala 3 opaque types, particularly those used in critical performance paths like the main render loop or physics engine.
