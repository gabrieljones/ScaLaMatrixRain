  ...
  def runLoop(options: Options, terminal: Terminal, sets: SetsOfCodePoints, colorContext: ColorContext): Unit = {
    ...
    val frameFn: Runnable = () => {
      ...
      while (dI < dropsLength) {
        ...
        // Optimization: Only update the current cell and generate a character if the drop has actually moved
        if (pXN != pXC || pYN != pYC) {
          val charIndex = nextBounded(setsLength)
          updateChar(pXN, pYN, charIndex, 0, cols, rows)
          updateChar(pXC, pYC, charIndex, 1, cols, rows)
        } else {
          // Stationary sub-frame drop: just preserve its state 0 to prevent fading without rendering overhead
          if (pXC >= 0 && pXC < cols && pYC >= 0 && pYC < rows) {
            val idx = pYC * cols + pXC
            colorBuffer(idx) = 0
          }
        }
        {
