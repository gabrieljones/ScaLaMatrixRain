...
## 2026-03-10 - [Optimization Success: Deferring RNG and Character Updates for Sub-Frame Drops]
**Learning:** In the drops advancement loop (`Main.scala`), drops with sub-unit velocity might not move every frame. Previously, `nextBounded(setsLength)` and terminal state updates were executed regardless of whether the drop moved. By checking `pXN != pXC || pYN != pYC` *before* generating the random index and calling `updateChar`, we save CPU cycles for stationary sub-frame drops. We just preserve their state at 0 to prevent fading without rendering overhead. This yielded significant FPS improvements (e.g., from ~2700 FPS to ~3200 FPS).
**Insight:** Deferring expensive operations (RNG and terminal array writes) until mathematically necessary avoids redundant work when an entity hasn't changed physical state.
**Action:** When updating entities that move at sub-frame velocities, wrap their visual state updates and RNG in a condition checking if their position actually changed this frame.
