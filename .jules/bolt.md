...
## 2026-03-30 - [Optimization Success: Deferring Random Generation in Drop Advancement]
**Learning:** In the drop advancement loop of `Main.scala`, a random character index was generated unconditionally at the beginning of the iteration for each drop (`val charIndex = nextBounded(setsLength)`), before computing the new position `(pXN, pYN)`. However, drops may not move every frame depending on their velocity (`vX, vY`). If a drop remains stationary (`pXC == pXN && pYC == pYN`), the random index is functionally ignored because `updateChar` is only conditionally called for the faded trail if the position has changed. Generating random numbers is relatively expensive even with a fast LCG.
**Insight:** Evaluating the conditional move (`pXC != pXN || pYC != pYN`) *before* invoking random number generation avoids wasted CPU cycles on stationary drops.
**Action:** Relocate the `nextBounded` random generation call *after* the movement calculation block and wrap it in the movement condition: `val charIndex = if (pXC != pXN || pYC != pYN) nextBounded(setsLength) else 0`. This small logical reordering avoids redundant RNG execution for slow-moving drops.
