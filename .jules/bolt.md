...
## 2026-06-16 - Array of Structures to Structure of Arrays
**Learning:** Re-structuring the 1D interleaved drops array (`dropsFlattened`) into 4 separate parallel arrays (`dropsX`, `dropsY`, `dropsVX`, `dropsVY`) improved caching locality and avoided mathematical operations on loop indices (like `i*4 + offset`). Combined with explicitly extracting array reads into local variables, performing explicit fast-path primitive operations (avoiding modulo for velocity = 1 or -1) and avoiding branching within `updateChar`, this led to significant rendering FPS gains in the main benchmark.
**Action:** When iterating over packed primitive arrays in Scala, extract into parallel SoA structures to improve CPU striding and eliminate division/modulo whenever variables are predominantly 1, 0, or -1.
