...
## 2026-03-05 - [Optimization Success: Deferring RNG and Avoid Fading for Stationary Sub-frame Drops]
**Learning:** In `Main.scala`'s drop advancement loop, if a drop has not actually moved within a single sub-frame (`pXN == pXC && pYN == pYC`), avoiding the bounded random generation (`nextBounded`) and explicitly skipping the `updateChar` I/O redraws yields ~25-30% performance boost (from ~2600 to ~3300 FPS).
**Insight:** Setting the drop's position in `colorBuffer` back to 0 without forcing a visual redraw allows the drop to retain its visual state without triggering costly fade operations or terminal drawing routines.
**Action:** Always verify if a particle/struct has moved before calculating new drawing properties or pushing to the graphics buffer. Update backing data buffers (like `colorBuffer`) directly to "pin" a state when actual redrawing is unnecessary.
