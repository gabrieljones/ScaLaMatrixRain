...
## 2026-04-15 - [Optimization Success: Defer RNG and Avoid Redraw for Stationary Drops]
**Learning:** In `Main.scala`, drops advanced based on velocity might not change position every frame (e.g., if `vX` or `vY` requires waiting multiple frames). By checking if a drop has actually moved (`pXN != pXC || pYN != pYC`) before generating a random character index and calling `updateChar`, we avoid unnecessary RNG and terminal output operations. If the drop is stationary, we just reset its `colorBuffer` state to `0` to prevent it from fading out. This yields an FPS improvement of ~10% (~2600 to ~2960 FPS).
**Insight:** Avoiding redundant work (random number generation and terminal draw calls) for stationary objects in a physics simulation loop yields huge performance benefits. Directly manipulating the state array (`colorBuffer`) is much cheaper than performing full character redraws via the terminal API.
**Action:** When objects update conditionally based on velocity or timers, wrap expensive initialization and draw logic in a check to ensure the object has actually changed state or position. Use fast-paths to maintain state without full redraws when possible.
